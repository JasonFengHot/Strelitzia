...
public class HistoryFavoriteActivity extends BaseActivity implements View.OnClickListener, OnItemFocusedListener, LfListItemClickListener, View.OnHoverListener, OnItemOnhoverlistener, OnItemKeyListener, View.OnKeyListener {
	...
	private void initView() {
		...
		historyRecycler.setOnHoverStateChangedListener(new HistoryRecyclerViewTV.OnHoverStateChangedListener() {
			@Override
			public void onHoverStateChanged(boolean hovered) {
				if (historyRecycler.getScrollState() == RecyclerView.SCROLL_STATE_IDLE) {
					arrowState();
				}
			}
		});
		...
		favoriteRecycler.setOnHoverStateChangedListener(new HistoryRecyclerViewTV.OnHoverStateChangedListener() {
			@Override
			public void onHoverStateChanged(boolean hovered) {
				if (favoriteRecycler.getScrollState() == RecyclerView.SCROLL_STATE_IDLE) {
					arrowState();
				}
			}
		});
...
//        }
	...
	protected void onDestroy() {
		if (favoriteRecycler != null) {
			favoriteRecycler.setOnHoverStateChangedListener(null);
			if (mOnScrollListener != null) {
				favoriteRecycler.removeOnScrollListener(mOnScrollListener);
			}
		}
		if (historyRecycler != null) {
			historyRecycler.setOnHoverStateChangedListener(null);
			if (mOnScrollListener != null) {
...
//                mDataCollectionProperties.put("position", history.get/1000);
		...
		protected Void doInBackground(Void... params) {
			try {
				...
				if (mHistories.size() > 0) {
					...
					for (History history:
						 mHistories) {
						Log.i("listSize", "time: " + history.add_time + "");
					...
					sortHistoryFavoriteList(allhistoryLists, historyLists);
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	class GetFavoriteTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			sortHistoryFavoriteList(allfavoriteLists, favoriteLists);
			return null;
		}
	...
	private void sortHistoryFavoriteList(List<HistoryFavoriteEntity> list, List<HistoryFavoriteEntity> list2) {
		int count = 0;
		if (list.size() > 0) {
