...
public final class HttpLoggingInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
         ...
         * ----------------------------------------
         */
        String port = request.url().port() == 80 ? "" : String.valueOf(request.url().port());
        logContent.setRequest_url(
                request.url().isHttps() ? "https://" : "http://"
                        + request.url().host()
                        + port
                        + request.url().encodedPath());
        logContent.setMethod(request.method());
        ...
        if (logHeaders) {
            ...
            if (request.method().equals("GET")) {
                ...
                for (String name : request.url().queryParameterNames()) {
                    requestParametersMap.put(name, request.url().queryParameter(name));
                }
        ...
        if (logHeaders) {
            ...
            if (!logBody || !HttpHeaders.hasBody(response)) {
                ...
                if (contentLength != 0) {
                    logger.log("");
                    logContent.setResult_type("参数");
                    String result = buffer.clone().readString(charset);
                    logContent.setRequest_result(result);
                    logger.log(result);
                }
    ...
    private HashMap<String, String> requestParamsToMap(String str) {
