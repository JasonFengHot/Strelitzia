//
// Created by huibin on 21/07/2017.
//
...
char *main_start(char hostname[]) {
    //Get the DNS servers from the resolv.conf file
    get_dns_servers();

    //Get the hostname from the terminal
//    LOGD("Enter Hostname to Lookup : ");
//    scanf("%s", hostname);
...
#ifdef DEBUG
            LOGD("has alias name : %s", answers[i].rdata);
#endif
        }
...
#ifdef DEBUG
        LOGD("Name : %s ", auth[i].name);
#endif

        if (ntohs(auth[i].resource->type) == 2) {
#ifdef DEBUG
            LOGD("has nameserver : %s", auth[i].rdata);
#endif
        }
...
/*
 * 获取android客户端dns服务器地址
 * */
void get_dns_servers() {
    char *dns1 = (char *) malloc(16);
    char *dns2 = (char *) malloc(16);
    __system_property_get("net.dns1", dns1);
    __system_property_get("net.dns2", dns2);
    strcpy(dns_servers[0], dns1);
    strcpy(dns_servers[1], dns2);
    free(dns1);
    free(dns2);
}
...
char *getByName(char hostName[]) {
    if (inet_addr(hostName) == INADDR_NONE) {
        //域名
        return main_start(hostName);
    } else {
        //ip地址
        return hostName;
    }
