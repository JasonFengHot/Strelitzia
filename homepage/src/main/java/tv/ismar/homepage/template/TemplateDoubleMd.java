...
//	private View mHeaderView; // recylview头view
	...
	/*modify by dragontec for bug 4434 start*/
		int selectedItemOffset = mContext.getResources().getDimensionPixelSize(R.dimen.banner_item_setSelectedItemDoubleOffset);
...
//  }
	...
	public void onItemClick(View view, int position) {
		if (position == 0) { // 第一张大图
			...
			BannerPoster entity = mAdapter.getData().get(position - 1);
			int Y = locationY;
			if (position % 2 != 0) {
				Y = locationY - 1;
			}
			...
			if (position < mAdapter.getItemCount())
				mFetchControl.launcher_vod_click(entity.model_name, entity.pk + "", entity.title, Y + "," + locationX, mChannel);
		}
	}
	...
	public void onClick(View v) {
		...
		if (i == R.id.navigation_left) {
			...
			if (position > 0) {
				...
				int targetPosition = position - 12;
				if (targetPosition < 0) {
					targetPosition = 0;
				}
...
//			mDoubleLayoutManager.setCanScroll(true);
//			mRecyclerView.loadMore();
//			if (positions[1] <= mDoubleLayoutManager.getItemCount() - 1) {
//				int targetPosition = positions[1] + 12;
//				if (targetPosition > mDoubleLayoutManager.getItemCount() - 1) {
//					targetPosition = mDoubleLayoutManager.getItemCount() - 1;
//				}
